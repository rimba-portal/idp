...
class UsersController
{
    ...
    {
        $model = config('idp.models.user');
        $users = $model::query()->with(config('idp.staff_relation', 'staff'))->paginate(100);

        return response()->json($users->through(static function ($user): array {
            $staff = data_get($user, config('idp.staff_relation', 'staff'));
...
class PermissionsController
{
    public function __invoke(): JsonResponse
    {
        $model = config('idp.models.permission');
